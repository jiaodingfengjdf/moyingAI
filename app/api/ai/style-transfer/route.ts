...
import { STYLE_TARGETS, buildStyleMessages, mockStyleText, type StyleTarget } from '@/lib/ai/style';
...
export async function POST(req: NextRequest) {
  ...
  const target = body?.target as StyleTarget;
  if (!chapterId) return NextResponse.json({ error: 'chapterId 必填' }, { status: 400 });
  if (!(target in STYLE_TARGETS)) return NextResponse.json({ error: '不支持的文风目标' }, { status: 400 });
  if (!sourceText.trim()) return NextResponse.json({ error: '请先选中文本' }, { status: 400 });
  ...
  try {
    ...
    const request = createAIRequest({ projectId: chapter.projectId, chapterId, kind: 'style', model: config.model, prompt: sourceText.slice(0, 300) });
    const mock = process.env.INKPULSE_AI_MOCK === '1';
    const stream = mock
      ? mockTextStream(mockStyleText(target))
      : await streamChat({ messages: buildStyleMessages(target, sourceText) }, config);
