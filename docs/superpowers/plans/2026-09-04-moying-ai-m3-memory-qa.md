...
Expected: `npm test` 全部 PASS（M1+M2 51 个 + 关系 2、伏笔 2、一致性 4，共 59 个用例）；lint 通过；build 成功。

- [ ] **Step 2: mock 模式浏览器验收清单**

- [x] 左栏可新建/编辑/删除关系（好感度滑杆、类型、备注），列表展示 A→B·类型·好感度。
- [x] 左栏可新建/编辑/删除伏笔；把回收区间上界设得小于当前章节数时出现黄色「⚠ 遗忘伏笔」。
- [x] 右栏「角色状态」展示实体最新时间线状态。
- [x] 修改正文保存后约 3 秒自动出现「一致性警报」；「重新检查」按钮可手动触发；无冲突时显示「未发现冲突」。
- [x] 刷新后关系、伏笔、警报状态均持久化。
...
1. **角色状态面板合并字段**：初版只展示时间线最新变更，发现后改为「实体卡字段 + 最新时间线」合并（与一致性引擎的 `latestStatus` 语义一致），实测 LinYan 显示 `level=qi-1`。
2. **伏笔 PATCH 空值收窄**：`'key' in body` 不能收窄 `body` 可空类型，改为局部 `const data = body ?? {}`（生产构建 TS 校验暴露）。
3. **一致性测试断言笔误**：含「复活」提示词的正文应判 0 条冲突，初稿断言 1 条，修正后 amend 提交。
4. **测试总数**：一致性用例实际 4 个（计划初稿误写 5），全量 59。
5. **超期语义**：判定为「全书章节总数严格大于区间上界」，与实现和测试一致。

验证证据：`npm test` 59/59 通过；`npm run lint` 无告警；`npm run build` 编译成功；`INKPULSE_AI_MOCK=1` 下浏览器完成「关系列表/表单、伏笔看板 ⚠ 遗忘伏笔、角色状态面板、保存后自动一致性检查并显示模拟冲突、重载持久化」全链路实测。
