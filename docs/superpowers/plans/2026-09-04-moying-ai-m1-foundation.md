...
- [x] 首页能创建项目并进入工作台；刷新后项目仍在。
- [x] 工作台能创建卷、章节、重命名、删除；章节切换顺畅。
- [x] 输入正文后 500ms 内顶栏出现「保存中…」并最终「已保存」；字数实时更新。
- [x] 停止并重启 `npm run dev` 后，正文与目录完整恢复（数据在 `data/app.db`）。
- [x] 快照可创建（带标签）、对比（红/绿高亮）、回滚（自动生成「回滚前自动快照」）、删除。
- [x] 选中正文出现悬浮菜单（按钮置灰，M2 接入）。
- [x] 删除项目后，其卷/章/快照在数据库中全部消失。
...
1. **`@types/node` 升级到 ^24**：Vitest 5 与 `node:sqlite` 类型要求 Node 22+，脚手架默认的 ^20 无法满足。
2. **快照测试预期修正**：回滚只新增 1 条「回滚前自动快照」，总数应为 2 而非 3（实现正确、测试笔误）。
3. **`lib/markdown.ts` 联合类型收窄修正**：`BlockNode.type` 为 string，`=== 'text'` 不能排除 BlockNode，改用显式断言（生产构建的 TS 校验暴露）。
4. **去除 `window.prompt/confirm/alert`**：Codex 内嵌浏览器不支持原生弹窗（`prompt() is not supported`），全部改为内联输入框与「两步确认」按钮。
5. **侧栏操作按钮改为常显**：原「悬停才显示」在无 hover 的触控/内嵌环境不可用，改为始终可见。
6. **回滚后状态同步修复**：回滚成功后同步重置顶栏字数，并刷新右栏快照列表（原实现有竞态）。

验证证据：`npm test` 34/34 通过；`npm run lint` 无告警；`npm run build` 编译成功；浏览器端完成「创建项目→卷/章→输入自动保存→选中悬浮菜单→快照/对比/回滚→重载持久化」全链路实测（项目「UI验收测试」保留在本地 `data/app.db` 中作示例数据）。
